...
$loginPortal = isset($loginPortal) && in_array($loginPortal, ['admin', 'player'], true) ? $loginPortal : 'player';
$isAdminPortal = $loginPortal === 'admin';
...
?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#153e13"><title><?= $isAdminPortal ? 'Admin sign in' : 'Player sign in' ?> | Laguerta Picklebook</title><link rel="stylesheet" href="assets/style.css?v=20260829-4"></head><body class="auth-body"><main class="auth-shell"><section class="auth-card simple-login"><a class="brand" href="<?= $isAdminPortal ? 'admin-login.php' : 'player-login.php' ?>"><span>LP</span><span class="brand-copy">Laguerta Picklebook<small><?= $isAdminPortal ? 'ADMINISTRATION' : 'COURT RESERVATIONS' ?></small></span></a><p class="eyebrow"><?= $isAdminPortal ? 'Private access' : 'Welcome' ?></p><h1><?= $isAdminPortal ? 'Admin sign in' : 'Player sign in' ?></h1><p><?= $isAdminPortal ? 'Enter the private administrator account details.' : 'Sign in to reserve a court and manage your bookings.' ?></p><?php if($error):?><div class="notice error"><?=loginEscape($error)?></div><?php endif;?><?php if(!$isAdminPortal && GoogleOAuth::configured()):?><a class="google-button" href="google-login.php"><span class="google-mark">G</span><strong>Continue with Google</strong><i>→</i></a><div class="signin-divider"><span>or sign in with your account</span></div><?php endif;?><form method="post" class="reservation-form signin-form"><input type="hidden" name="csrf_token" value="<?=loginEscape(Auth::csrf())?>"><label>Username or email<input name="identity" autocomplete="username" placeholder="Enter username or email" required></label><label>Password<input type="password" name="password" autocomplete="current-password" placeholder="Enter password" required></label><button class="primary-button" type="submit">Sign in <span>→</span></button></form><?php if(!$isAdminPortal):?><p class="auth-link">New player? <a href="register.php">Create an account</a></p><?php endif;?></section></main></body></html>
