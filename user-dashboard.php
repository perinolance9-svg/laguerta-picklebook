...
$reservations=array_values(array_filter($service->getReservationsByUser($userId),function(array $reservation)use($now):bool{
...
$dailyStatement=$connection->prepare("SELECT r.reservation_id,r.user_id,r.start_time,r.end_time,r.status,CONCAT(u.first_name,' ',u.last_name) player_name,c.court_name,c.location FROM reservations r INNER JOIN users u ON u.user_id=r.user_id INNER JOIN courts c ON c.court_id=r.court_id WHERE r.reservation_date=:schedule_date AND r.status<>'Cancelled' ORDER BY r.start_time,c.court_name");
...
<section class="daily-schedule"><div class="daily-schedule-head"><div><p class="eyebrow">Daily schedule</p><h2>All bookings for <?=udEscape(date('M j, Y',strtotime($scheduleDate)))?></h2><p>See who booked each court and every reserved time for the selected day.</p></div><form method="get" class="daily-date-filter"><label>Choose date<input type="date" name="schedule_date" value="<?=udEscape($scheduleDate)?>" onchange="this.form.submit()"></label></form></div><div class="daily-booking-grid"><?php if(!$dailyBookings):?><p class="empty daily-empty">No courts are booked on this date.</p><?php endif;?><?php foreach($dailyBookings as $daily):?><article class="daily-booking-card"><div><small><?=udEscape($daily['location'])?></small><h3><?=udEscape($daily['court_name'])?></h3><small>Booked by <?=udEscape($daily['player_name'])?></small></div><strong><?=date('g:i A',strtotime($daily['start_time']))?>–<?=date('g:i A',strtotime($daily['end_time']))?></strong><span class="status status-<?=strtolower(udEscape($daily['status']))?>"><?=udEscape($daily['status'])?></span></article><?php endforeach;?></div></section>
...
<article class="panel role-reservations"><div class="role-section-head"><div><p class="eyebrow">My schedules</p><h2>My reservations</h2></div><span id="booking-count"><?=count($reservations)?> bookings</span></div><div class="booking-cards" id="booking-cards"><?php if(!$reservations):?><p class="empty">You have no current or upcoming reservations.</p><?php endif;?><?php foreach($reservations as $r):?><div class="booking-card"><div class="booking-date"><strong><?=date('d',strtotime($r['reservation_date']))?></strong><span><?=date('M',strtotime($r['reservation_date']))?></span></div><div><strong><?=udEscape($r['court_name'])?></strong><small><?=udEscape($r['location'])?> · <?=date('g:i A',strtotime($r['start_time']))?>–<?=date('g:i A',strtotime($r['end_time']))?></small></div><div class="booking-state"><span class="status status-<?=strtolower(udEscape($r['status']))?>"><?=udEscape($r['status'])?></span><small><?=udEscape($r['payment_status']??'Not paid')?></small><?php if($r['status']==='Pending'&&($r['payment_status']??'')!=='For Verification'):?><a class="pay-button" href="personal-gcash-payment.php?reservation_id=<?=udEscape($r['reservation_id'])?>">Pay now</a><?php endif;?></div></div><?php endforeach;?></div></article></section><?php endif;?></main>
<script>
...
refreshCourts();refreshBookings();setInterval(tickCourts,1000);setInterval(refreshCourts,5000);setInterval(refreshBookings,5000);document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'){refreshCourts();refreshBookings();}});
